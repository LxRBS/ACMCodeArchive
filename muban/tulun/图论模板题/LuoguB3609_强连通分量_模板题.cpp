...
void proc(){
    SCC.Tarjan();
    vector<vector<int>> vec(SCC.SCCCnt);
    for(int i=1;i<=N;++i){
        vec[SCC.Belong[i]].emplace_back(i);
    }
    cout << SCC.SCCCnt << endl;
    vector<int> flag(SCC.SCCCnt, 0);
    for(int i=1;i<=N;++i){
        if(flag[SCC.Belong[i]]) continue;
        flag[SCC.Belong[i]] = 1;
        for(auto j : vec[SCC.Belong[i]]) cout << j << " ";
        cout << endl;
    }
    return;
}
