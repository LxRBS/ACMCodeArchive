...
void proc(){
    auto ans = ISAP.maxflow(S, T);
    if(ans != Total) return (void)(cout << 0 << endl);

    cout << 1 << endl;
    vvi results(M + 1, vi());
    ISAP.forEachUsed([&](int p, MaxFlow_ISAP::weight_t usedw){
        int from = ISAP.edges[p ^ 1].to;
        int to = ISAP.edges[p].to;
        if(1 <= from and from <= M and M + 1 <= to and to <= M + N){
            assert(1 == usedw);
...
int main(){
    ...
    cin >> M >> N;
    Total = 0;
    ISAP.init(T = 1 + (S = N + M + 1));
    for(int r,i=1;i<=M;++i){
        cin >> r;
        ISAP.mkDirectEdge(S, i, r);
        Total += r;
    }
    for(int r,i=1;i<=N;++i){
        cin >> r;
        ISAP.mkDirectEdge(i + M, T, r);
    }
    for(int i=1;i<=M;++i)for(int j=1;j<=N;++j){
        ISAP.mkDirectEdge(i, j + M, 1);
    }
    proc();
    return 0;
}