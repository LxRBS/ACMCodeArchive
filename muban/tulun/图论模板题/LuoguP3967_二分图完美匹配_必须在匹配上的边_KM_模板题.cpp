/**
 * 完美匹配，以及求必须在完美匹配上的边，保证完美匹配存在
 * 首先求出完美匹配。
 * 然后对完美匹配的每条边，删边再求完美匹配。如果不能达到最大值，则改边必须存在
 */
#include <bits/stdc++.h>
using namespace std;
...
void proc(){
    ...
    for(int i=1;i<=N;++i) vec.emplace_back(i, KM.linka[i]);

    vector<pair<int, int>> ret; ret.reserve(N);
    for(const auto & p : vec){
        KM.init(N);
        for(int i=0;i<N;++i)for(int j=0;j<N;++j){
            if(i + 1 == p.first and j + 1 == p.second) continue;
            KM.mkDiEdge(i + 1, j + 1, W[i][j]);
        }        
        if(KM.KM() < ans) ret.push_back(p);
    } 
    cout << ans << endl;
    for(const auto & p : ret) cout << p.first << " " << p.second << endl;
    return;
}
