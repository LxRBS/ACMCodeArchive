...
/** 7 Graham凸包算法 **/
/// 求点集p的凸包，会改变p的内容
/// 结果放在p[0..r)中，其中r是返回值，表示凸包的点的数量，逆时针排列
...
/// 返回最小面积, pans中记录4个点，为矩形
/// p保证是逆时针, 顺时针需要更改while中的关系符号
Real rcRecatangle(const Point p[], int n, Point pans[] = nullptr){
    if(1 == n){ // 一个点的特殊情况
        return 0;
    }
    if(2 == n){ // 二个点的特殊情况
    ...
    while(t < n && cross(p[0], p[1], p[t]) <= cross(p[0], p[1], p[(t+1)%n])){
        ++t;
    }
    if(t == n){ /// 说明都在一条直线上
        return 0;
    ...
    for(int i=0;i<n;++i){
        /// 找另外三个卡点
        const Vector vec = p[(i+1)%n] - p[i];
        while(dot(p[(r+1)%n]-p[r], vec) >= 0){
            r = (r + 1) % n;
        }
        while(cross(p[i], p[(i+1)%n], p[t]) <= cross(p[i], p[(i+1)%n], p[(t+1)%n])){
            t = (t + 1) % n;
        }
        while(dot(p[(l+1)%n]-p[l], vec) <= 0){
            l = (l + 1) % n;
        }
        /// 矩形为(i,i+1)边所在的直线且卡住t,l,r三个点
        /// 矩形的面积就是t到(i,i+1)的距离乘以向量(l, r)在(i,i+1)上投影的长度
        Real chang = fabs(cross(p[i], p[(i+1)%n], p[t]) / vec.length()); 
        Real kuan = fabs(dot(vec, p[r] - p[l]) / vec.length());
        Real tmp = chang * kuan;
        if(tmp < ans){
            ans = tmp;
            /// 记录点
            if(pans != nullptr){
                /// 底部的直线
                ArgLine bLine(Point(p[i].x, p[i].y), Vector(p[(i+1)%n].x - p[i].x, p[(i+1)%n].y - p[i].y));
                /// 右侧的直线
                ArgLine rLine(Point(p[r].x, p[r].y), Vector(-bLine.direction.y, bLine.direction.x));
                /// 上部直线
                ArgLine tLine(Point(p[t].x, p[t].y), bLine.direction);
                /// 左侧直线
                ArgLine lLine(Point(p[l].x, p[l].y), rLine.direction);
                /// 4个交点
                int c919 = relation(bLine, rLine, pans);
                c919 &= relation(rLine, tLine, pans+1);
                c919 &= relation(tLine, lLine, pans+2);
                c919 &= relation(lLine, bLine, pans+3);
                assert(INTER & c919);
            }
        }
    }
