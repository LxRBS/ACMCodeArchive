...
pll St[SIZE<<2]; // (sum, peak)

inline void _pushUp(int t){
	const pll &left = St[lson(t)];
	const pll &right = St[rson(t)];
	pll &p = St[t];
...
llt qPeak(int t, int s, int e, int a, int b){
	if(a<=s && e<=b){
		return St[t].second;
	}
	...
	int mid = (s + e) >> 1;
	llt ans = -1E15;
	if(a<=mid) ans = max(qPeak(lson(t), s, mid, a, b), ans);
	if(mid<b) ans = max(qPeak(rson(t), mid+1, e, a, b), ans);
	return ans;	
}

void modify(int t, int s, int e, int a, int b, llt newValue){
    if(s == e){
		St[t] = {newValue, newValue};
		return;
	}
...
void modify(int u, llt newValue){
	modify(1, 1, N, Node[u].nid, Node[u].nid, newValue);
}

llt querySum(int u, int v){
	llt ret = 0;
...
llt queryPeak(int u, int v){
	llt ret = -1E15;
    while(Node[u].top != Node[v].top){
        if(Node[Node[u].top].depth < Node[Node[v].top].depth) swap(u, v);

		ret = max(qPeak(1, 1, N, Node[Node[u].top].nid, Node[u].nid), ret);
		u = Node[Node[u].top].parent;
	}
	if(Node[u].depth  > Node[v].depth) swap(u, v);
	ret = max(qPeak(1, 1, N, Node[u].nid, Node[v].nid), ret);
	return ret;	
}
