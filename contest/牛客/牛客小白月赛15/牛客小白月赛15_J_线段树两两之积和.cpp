...
void _dn_(int t, int s, int e, const lazy_type & delta) {
    ...
    auto n = e - s + 1;
    data[t].first = sum + n * delta;
    data[t].second = s2 + n * delta * delta + 2LL * delta * sum;
    lazy[t] += delta;
}
...
void _build(int t, int s, int e) {
    if(s == e) {
        int x; cin >> x;
        data[t] = value_type(x, x * x); // 注意提供value_type的初始化
        return; 
    }
    int mid = (s + e) >> 1;
    _build(lson(t), s, mid);
...
int main(){
    ...
    for(int c,a,b,x,q=1;q<=Q;++q){
        ...
        if(1 == c){
            cin >> x;
            St.modify(1, 1, N, a, b, x);
        }else if(2 == c){
            auto ans = St.query(1, 1, N, a, b);
            cout << (ans.first * ans.first - ans.second) / 2 << endl;
        }else{
            assert(0);
        }
    }
