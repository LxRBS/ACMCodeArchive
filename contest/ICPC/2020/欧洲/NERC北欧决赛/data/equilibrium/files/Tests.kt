...
private fun solveGreedy(n: Int, xb: Double, yb: Double): IntArray {
	val a = IntArray(n)
	while (true) {
...
Tests byIndex 16 2000000 > $
Tests byIndex 17 2000000 > $
Tests byIndex 18 2000000 > $
Tests byIndex 18 200000000 > $
Tests byIndex 18 477638699 > $
Tests byIndex 18 477638698 > $
Tests byIndex 18 477630700 > $
Tests byIndex 18 477608700 > $
Tests byIndex 18 477038700 > $
Tests byIndex 18 470638700 > $
Tests byIndex 18 407638700 > $
Tests byIndex 18 307638700 > $
Tests byIndex 18 207638700 > $
Tests byIndex 18 107638700 > $
Tests byIndex 18 566 > $
Tests byIndex 18 1 > $
Tests byIndex 18 0 > $
Tests hills 3 3 3 3 3 3 > $
Tests hills 6 6 6 > $
Tests hills 6 12 > $
Tests hills 13 5 > $
Tests hills 1 17 > $
Tests hills 16 2 > $
Tests greedyDegrees 18 0 > $
Tests greedyDegrees 18 10 > $
Tests greedyDegrees 18 30 > $
Tests greedyDegrees 18 45 > $
Tests greedyDegrees 18 70 > $
Tests greedyDegrees 18 89 > $
Tests greedyDegrees 18 92 > $
Tests greedyDegrees 18 111 > $
Tests greedyDegrees 18 163 > $
	"""
	...
	fun hills(hills: List<String>) {
		val a = mutableListOf<Int>()
		var h = 0
		for (x in hills.map { it.toInt() }) {
			repeat(x) { a.add(h) }
			h += x
		}
		val (xc, yc) = toNonDiagonal(centerOfMassBrackets(a.toIntArray()))
		test(a.size, xc, yc)
	}

	fun greedyDegrees(parameters: List<String>) {
		val (nIn, alphaIn) = parameters
		val n = nIn.toInt()
		val alpha = (alphaIn.toInt() + 45) * PI / 180
		val a = solveGreedy(n, 8 * n * cos(alpha), 8 * n * sin(alpha))
		val (xc, yc) = toNonDiagonal(centerOfMassBrackets(a))
		test(n, xc, yc)
	}
...
//		validateTest(n, xPrinted.toDouble(), yPrinted.toDouble())
	...
	private fun validateTest(n: Int, xc: Double, yc: Double) {
		val solution = solveOptimized(n, xc, yc)
//		println(solution)
