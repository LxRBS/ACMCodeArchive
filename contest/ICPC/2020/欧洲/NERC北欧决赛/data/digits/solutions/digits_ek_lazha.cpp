...
public:
    void solve() {
        ...
        arr<ll> best(10, numeric_limits<ll>::max());
        arr<vi> vals(10, vi());
        arr<ll> nbest(10, numeric_limits<ll>::max());
        arr<vi> nvals(10, vi());
        vi good;
        best[1] = 1;
        int valsP = 1;
        for (int i : a) {
            ...
            valsP %= 10;
            good.push_back(i);
            copy(all(best), nbest.begin());
            copy(all(vals), nvals.begin());
            for (int j : range(10)) {
                if (best[j] != numeric_limits<ll>::max() && best[j] * i < nbest[j * i % 10]) {
                    nbest[j * i % 10] = best[j] * i;
                    nvals[j * i % 10] = vals[j];
                    nvals[j * i % 10].push_back(i);
                }
            }
            swap(vals, nvals);
            swap(best, nbest);
        }
        ll totBest = numeric_limits<ll>::max();
        vi answer;
        for (int i : range(10)) {
            if (d * i % 10 == valsP && totBest > best[i]) {
                multiset<int> s;
                vi canswer;
                for (int j : vals[i]) {
                    s.insert(j);
        ...
        if (totBest == numeric_limits<ll>::max()) {
            out.printLine(-1);
            return;
        }
