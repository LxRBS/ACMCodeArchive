...
bool extend(vp &P, vp &edge, int sgn) {
	...
	for (int a = 0, b = edge.size()-1; a < edge.size(); b = a++)
		if (inside(edge[a], P) == sgn && inside(edge[b], P) != sgn) {
			vp add;
			for (int i = 0; inside(edge[(a+i)%n], P) == sgn; ++i)
				add.push_back(edge[(a+i)%n]);;
			int idx = find_pos(P, edge[a], edge[b]);
			dprintf("add from %c(%d,%d) count %d [prev %c(%d,%d)] at pos %d (btw %c(%d,%d), %c(%d,%d))\n",
			...
			P.insert(P.begin()+idx, add.begin(), add.end());
			changed = true;
		}
	if (changed) {
		dprintf("after extend %d\n", sgn);
		print(P);
	}
	return changed;
}
