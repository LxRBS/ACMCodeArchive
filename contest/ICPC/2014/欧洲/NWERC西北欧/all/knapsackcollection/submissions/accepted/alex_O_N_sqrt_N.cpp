...
int gNext[MAXN]; // next position to shoot at - it is not updated in this implementation -> NO path compression
bool removed[MAXN]; // specifies whether this target is already eliminated

int inBlock[MAXN];  // each id belongs to a consecutive block of ~sqrt(N) objects
int numInBlock[MAXN];// number of remaining items in a block
int nextBlock[MAXN];
int blockBegin[MAXN];
int blockEnd[MAXN];
...
// it uses NOT path compression
int getNext(int n){
	++FUNCCALLS;
	int b = inBlock[n];
	while(numInBlock[b] == 0){
		b = nextBlock[b];
		n = blockBegin[b];
	}
	if(removed[n]){
		return getNext(gNext[n]);
	}
	return n;
}
int N, P, R;
int main(int argc, char* argv[]) {
	...
	ll toGo = (R) % P;
	int SQ = max(1,(int)sqrt(N+eps));
	memset(blockBegin,oo,sizeof(blockBegin));
	FOR(i,0,N){
		int b = i / SQ;
		inBlock[i] = b;
		nextBlock[b] = b+1;
		blockBegin[b] = min(blockBegin[b], i);
		blockEnd[b] = i + 1;
	}
	int B = ((N-1) / SQ) + 1;
	nextBlock[B-1] = 0;
	...
	FOR(j,0,N){
		if(x[j] != laststart){ // calculate values for current start if not already done
			...
			FOR(i,0,B)numInBlock[i] = blockEnd[i] - blockBegin[i];
			numInBlock[inBlock[cur]]--;
			FOR(i,1,N){
