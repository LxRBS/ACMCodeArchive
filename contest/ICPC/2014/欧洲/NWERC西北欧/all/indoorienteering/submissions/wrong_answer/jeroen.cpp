...
void rec1(int i, int used, int t, int d) {
	if(t == 0) {
		if(i != midpoint) return;
		can.insert(pair<int,int>(used, d));
	} else if(t == 1) {
...
void rec2(int i, int used, int t, int d) {
	if(t == 0) {
		if(i != midpoint) return;
		possible |= can.find(pair<int,int>(used ^ ((1<<N)-1) ^ (1<<midpoint) ^ 1,d)) != can.end();
	} else if(t == 1) {
