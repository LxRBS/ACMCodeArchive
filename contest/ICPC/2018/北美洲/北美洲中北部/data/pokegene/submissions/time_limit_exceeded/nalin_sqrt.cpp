...
void solve()
{
	...
	if(len < BLOCK)
	{
		for(int i = 0;i < len;i++)
		{
			...
			{
				int lc = LCA(eve[i].second, eve[j].second);
	...
	{
		for(auto it: mappednodes) eve.push_back({{strt[it], 1}, it});
	}
