...
public class joined_vessels_bm_fast {
    FastScanner in;
    PrintWriter out;
    ...
    class SegmentTree {
        ...
        int findFirstLessOrEq(int v, int l, int r, int value) {
            if (min[v] > value) {
                return -1;
            }
            if (l == r) {
            ...
            relax(v, l, r);
            int m = (l + r) >> 1;
            int res = findFirstLessOrEq(v * 2 + 1, l, m, value);
            if (res != -1) {
                return res;
            }
            return findFirstLessOrEq(v * 2 + 2, m + 1, r, value);
        }
    ...
    void solve() {
        ...
        for (int rev = 0; rev < 2; rev++) {
            ...
            for (int i = 0; i + 1 < n; i++) {
                int curH = h[i];
                int r = st.findFirstLessOrEq(0, 0, n - 1, curH);
                st.set(0, 0, n - 1, r, i, curH);
                for (TestCase t : check[i]) {
                    long valueAtFrom = st.getSum(0, 0, n - 1, t.fr, t.fr);
                    int ri = st.findFirstLessOrEq(0, 0, n - 1, (int) valueAtFrom);
    ...
    public static void main(String[] args) {
        new joined_vessels_bm_fast().runIO();
    }
}