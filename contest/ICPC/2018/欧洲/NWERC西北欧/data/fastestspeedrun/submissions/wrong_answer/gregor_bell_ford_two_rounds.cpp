...
void update_costs_dfs(int i,int j){
	...
	b[i][j] = a[i][j];
	if (i == 0) b[i][j] = a[i][j];
	else if (adj[i][0][0] == 0) b[i][j] = 0;
	else {
		update_costs_dfs(adj[i][0][0],j);
		b[i][j] = min(b[i][j], b[adj[i][0][0]][j]);
	}
}
...
int main(){
	...
	FOR(i,0,n+1) FOR(j,0,n+1) update_costs_dfs(i,j);

	dp[0] = 0;
	FOR(i,1,n+1) dp[i] = ooo;

	FOR(k,0,2) FOR(i,0,n+1)	FOR(j,0,n+1) dp[j] = min(dp[j], dp[i] + b[j][i]);
