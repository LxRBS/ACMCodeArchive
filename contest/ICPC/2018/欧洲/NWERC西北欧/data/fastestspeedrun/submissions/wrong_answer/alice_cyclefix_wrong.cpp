...
void cycle_least_cost(ll cur1, ll cur2, vl& pre, vvl& cost, vvl& a, vector<vb>& visited){
	...
	while(true){
		cur = pre[cur];
		if(DEBUG) {cerr << cur << endl;}
		if(cur == -1){
			vals.insert(0);
			break;
		}
		if(visited[cur][cur2]){
			vals.insert(cost[cur][cur2]);
			break;
		}

		if(!(cycle.insert(cur)).second) break;
	}
	for(auto& c : cycle) vals.insert(a[c][cur2]);
	ll mi = *(vals.begin());
	for(auto& c : cycle){
		cost[c][cur2] = mi;
		visited[c][cur2] = true;
	}
	assert(visited[cur1][cur2]);
