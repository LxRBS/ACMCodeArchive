...
int main(){
    ...
    // Oops: we need to look at all of the items, not just ones inside concat()
    // (and at the ends, in this case).
    if (res1[i]->type == 3 or i < 4 or i+4 >= res1.size()){
      ...
      // Compare the set in this range for some silly reason.
      if (set<int>(res1[i]->v.begin(),res1[i]->v.end()) !=
          set<int>(res2[i]->v.begin(),res2[i]->v.end())){
        cout << "not equal" << endl;
        return 0;
      }
    }
