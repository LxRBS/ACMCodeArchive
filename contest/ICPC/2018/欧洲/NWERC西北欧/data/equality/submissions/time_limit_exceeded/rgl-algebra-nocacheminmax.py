...
class Concatenation:
  ...
  # Return raw elements in any order.
  def flatten(self):
    if self.a.ftype == Type.CONCAT: self.a = self.a.flatten()
    if self.b.ftype == Type.CONCAT: self.b = self.b.flatten()
    arr = None
    if len(self.a.arr) > len(self.b.arr):
      arr = self.a.arr
      arr.extend(self.b.arr)
    else:
      arr = self.b.arr
      arr.extendleft(reversed(self.a.arr))
