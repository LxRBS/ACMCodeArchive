...
bool proc(){
	...
	/// 确定两个平面的位置关系
	int r = relation(pa, pb);
	if(r & PINGXING){ // 平行肯定不想交
		return false;
	}
    if(r & CHONGHE){ // 按题意重合也不应该相交
		return false;
	}
	/// 求两个平面的交线
...
int main() {
	...
	for(int kase=1;kase<=nofkase;++kase){
        A.x = getInt(); A.y = getInt(); A.z = getInt();
		Na.x = getInt(); Na.y = getInt(); Na.z = getInt();
		Ra = getInt();
		B.x = getInt(); B.y = getInt(); B.z = getInt();
		Nb.x = getInt(); Nb.y = getInt(); Nb.z = getInt();
		Rb = getInt();
		puts(proc()?"Yes":"No");
	}
    return 0;
