...
//最大值函数 
int gmax(int depth,char ch){
    int i,j;
    if ( 17 == depth ) return 0;
    
    int best = -2;
    for(i=0;i<4;i++)for(j=0;j<4;j++){
		if ( '_' != Board[i][j] ) continue;
...
int main(){
	...
	if ( x_num == o_num ) ans = gmax(x_num+o_num+1,'X');
	else                  ans = gmax(x_num+o_num+1,'O');
