...
int const DIM_SQUARE = DIMENSION * DIMENSION;//9乘9的数独
char Input[20][20];
...
bool read(){
    if (EOF == scanf("%s",Input[0])) return false;
    for(int i=1;i<DIM_SQUARE;++i){
        Input[0][i+i-1] = ' ';
        scanf("%s",Input[0]+i+i);
    }
    ...
    for(int i=1;i<DIM_SQUARE;++i){
        ...
        Input[i][DIM_SQUARE+DIM_SQUARE-1] = '\0';
    }
