...
//to find how many times each pattern appears in the given target
int find(char const target[],int ans[]){
    int state = StartState;
    for(char const*p=target;*p;++p){
        state = Trie[state][L2I[(int)*p]];
        int t = state;
        while( t != StartState ){
            if ( Flag[t] ) ++ans[Flag[t]];
            //Flag[t] = -1;//can't do it here because AC is used for many times
            t = Fail[t];
        }
    }
    return 0;
