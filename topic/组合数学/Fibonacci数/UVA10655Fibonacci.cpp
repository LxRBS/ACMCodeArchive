/*
    已知a+b和ab的值，求a^n+b^n
    令  Fn = a^n + b^n
    则  Fn = ( a + b )( a^(n-1) + b^(n-1) ) - ab( a^(n-2) + b^(n-2) )
    所以 Fn = (a+b)F(n-1) - (ab)F(n-2)
*/
#include <stdio.h>
#include <string.h>
#include <algorithm>
...
int main(){
    ...
    while( 3 == scanf("%lld%lld%lld",&P,&Q,&N) ){
        F1 = A.data[0][0] = P;
        A.data[0][1] = -Q;
        F2 = P * P - 2LL * Q;
        ...
        matrix_t& ans = matrix_t::power(A,N-2);
        llt ret = ans.data[0][0] * F2 + ans.data[0][1] * F1;
        printf("%lld\n",ret);
    }
    return 0;
}
