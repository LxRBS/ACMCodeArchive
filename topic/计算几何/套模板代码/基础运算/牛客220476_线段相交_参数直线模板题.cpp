...
/// 不交叉相交返回true，交叉相交返回false
bool proc(const ArgLine &u, const ArgLine &v, Real tu, Real tv){
    Point p;
    int r = relation(u, v, &p);
    if((r & PINGXING) || (r & CHONGHE)) return true;

    Real t;
    r = relation(p, u, &t);
    assert(r & IN);
    assert(sgn(t) && sgn(t - tu));
    if(sgn(t) < 0 || sgn(t - tu) > 0) return true;

    r = relation(p, v, &t);
    assert(r & IN);
    assert(sgn(t) && sgn(t - tv));
    if(sgn(t) < 0 || sgn(t - tv) > 0) return true;
...
string f(){
    stringstream ss;
    for(int i=0;i<4;++i){
        ss << fixed << setprecision(3) << P[i].x << " " << P[i].y << " ";
    }
    return ss.str();
}
...
int main() {
    ...
    /// 题目保证数据不相等，但实际上有一组数据是相等的
    if(P[0] == P[1] || P[2] == P[3]) {
        return cout << 0 << endl, 0;
    }

    int r = relation(P[0], P[1], P[2], P[3]);

    ArgLine u(P[0], P[1] - P[0]);
    ArgLine v(P[2], P[3] - P[2]);
    Real tu = dist(P[0], P[1]), tv = dist(P[2], P[3]);

    if(proc(u, v, tu, tv)) {
        return cout << 0 << endl, 0;
    }

    Real ans = cross(P[0], P[2], P[1]) + cross(P[0], P[1], P[3]);
    cout << fixed << setprecision(12) << 0.5 * fabs(ans) << endl;
    return 0;
}
