...
int main() {
    ...
    for(int i=0;i<N;++i)for(int j=i+1;j<N;++j){
        ArgLine line(P[i], P[j] - P[i]);
        if(sgn(line.direction.x) < 0){
            line.direction.x = - line.direction.x;
            line.direction.y = - line.direction.y;
        }else if(is0(line.direction.x) && sgn(line.direction.y) < 0){
            line.direction.y = - line.direction.y;
        }
        vec.push_back(line.direction);
    }
    sort(vec.begin(), vec.end(), [](const Vector&a, const Vector &b)->bool{
        return sgn(cross(a, b)) < 0;
    });
    vec.erase(unique(vec.begin(), vec.end(), [](const Vector&a, const Vector &b)->bool{
        return is0(cross(a, b));
    }), vec.end());
