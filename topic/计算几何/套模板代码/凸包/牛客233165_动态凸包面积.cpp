/**
 * 给定一个凸包，再给定若干个点，问从这若干个点中选取哪一个构成新凸包，可以使得新凸包面积最大
 * 求这个最大面积
 * 凸包5w，点5w，需要使用log算法
 * 对每一个点，如果就在凸包内，则忽略
 * 否则求出其关于凸包的两个切点，该点带来的新面积就是这个三角形的面积减去凸包上弓形的面积
 * 凸包弓形的面积可以预处理出所有扇形的面积，然后O(1)求出
 */
#include <bits/stdc++.h>
using namespace std;
...
llt proc(){
    auto pp = Con.tangent(Q);
    llt tmp = Con.calcArcArea(pp.first, pp.second);
    return Q.cross(Con.pts[pp.second], Con.pts[pp.first]) - tmp;
}
...
int main(){
	...
	for(int i=K;i<N;++i){
        Q.input();
        if(IN & Con.relate(Q)) continue;
        ans = max(ans, proc());
	}
    ans += Con.sector_area[n];
