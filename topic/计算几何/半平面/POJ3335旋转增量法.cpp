...
int main(){
    ...
    while(nofkase--){
        ...
        //判断旋向
        int area = 0;
        for(int i=2;i<N;++i) area += cross(P[0],P[1],P[i]);

        //生成半平面
        P[N] = P[0];
        if ( area > 0 ) for(int i=0;i<N;++i) genHP(P[i+1],P[i],Hp[i]);
        else for(int i=0;i<N;++i) genHP(P[i],P[i+1],Hp[i]);

        printf(sandi(Hp,N,bot,top) > 2 ? "YES\n" : "NO\n");
    }
    return 0;
}
