...
bool read(){
    ...
    initGraph(N+N+F+D+2);
    //每个牛建两个点一条边
    for(int i=1;i<=N;++i) mkEdge(i,N+i,1);
    //源到每个F建一条边
    for(int i=1;i<=F;++i) mkEdge(N+N+F+D+1,N+N+i,1);
    //每个D到汇建一条边
    for(int i=1;i<=D;++i) mkEdge(N+N+F+i,N+N+F+D+2,1);
    //F到Ni建一条边，Ni到D建一条边
    for(int i=1;i<=N;++i){
        int a,b;
        scanf("%d%d",&a,&b);
