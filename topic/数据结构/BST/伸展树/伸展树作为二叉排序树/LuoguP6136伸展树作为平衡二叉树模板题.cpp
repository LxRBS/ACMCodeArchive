...
int main() {
    ...
    while(q--){
        int cmd = getInt();
        switch(cmd){
            case 1:Tree.insert(getInt()^last);break;
            case 2:Tree.erase(getInt()^last);break;
            case 3:ans^=last=Tree.rank(getInt()^last);break;
            case 4:ans^=last=Tree.select(getInt()^last).key;break;
            case 5:ans^=last=Tree.pred(getInt()^last).key;break;
            case 6:ans^=last=Tree.succ(getInt()^last).key;break;
            default:assert(0);
        }
    }
    printf("%d\n", ans);
